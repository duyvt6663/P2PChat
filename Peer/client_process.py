...
class Client(Frame):
    ...
    def initUI(self):
        ...
        self.root.geometry("%dx%d+%d+%d" % (self.FrameSizeX,
                           self.FrameSizeY, FramePosX, FramePosY))
        ...
        serverPortField = Entry(
            ipGroup, width=5, textvariable=self.serverPortVar)
        serverSetButton = Button(
            ipGroup, text="Set", width=10, command=self.handleSetServer)
        ...
        clientPortField = Entry(
            ipGroup, width=5, textvariable=self.clientPortVar)
        clientSetButton = Button(
            ipGroup, text="Add", width=10, command=self.handleAddClient)
        ...
        self.receivedChats = Text(
            readChatGroup, bg="white", width=60, height=30, state=DISABLED)
        ...
        self.chatField = Entry(writeChatGroup, width=80,
                               textvariable=self.chatVar)
        sendChatButton = Button(
            writeChatGroup, text="Send", width=10, command=self.handleSendChat)
    ...
    def listenClients(self):
        while 1:
            ...
            threading.start_new_thread(
                self.handleClientMessages, (clientsoc, clientaddr))
        self.serverSoc.close()
